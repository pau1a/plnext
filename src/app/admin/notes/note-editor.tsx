...
const initialNoteActionState = { status: "idle" as const };
...
> = {
  title: {
    label: "Title",
    type: "text",
    placeholder: "Introducing the Notes Stream",
  },
  date: {
    label: "Publish date",
    type: "date",
  ...
  summary: {
    ...
    type: "textarea",
    rows: 3,
    placeholder: "Short purpose-driven description of the note.",
  },
  tags: {
    label: "Tags",
    type: "tags",
  ...
  draft: {
    label: "Draft",
    type: "checkbox",
    helpText: "Draft notes stay off the public stream.",
  },
};
...
export function NoteEditor({ slug, title, initialBody, initialFrontmatter, frontmatterOrder }: NoteEditorProps) {
  ...
  const primaryKeys = ["title", "date", "summary", "tags"];
  ...
  return (
    <form className="u-stack u-gap-xl" action={formAction}>
            ...
            <div className="admin-essay-editor__statbar">
              <div className="admin-essay-editor__statrow">
                <span className="admin-essay-editor__stat">
                  Editing <code>content/notes/{slug}.mdx</code>
                </span>
                <span className="admin-essay-editor__stat">
                  Permalink <code>/notes/{derivedSlug}</code>
                </span>
              </div>
            </div>
          </div>
          <div className="u-flex u-gap-sm">
            <Link className="button button--ghost button--sm" href="/admin/notes">
              Back to list
            </Link>
            <Link className="button button--ghost button--sm" href={`/notes/${derivedSlug}`} prefetch={false}>
              View live
            </Link>
          </div>
        </div>
        ...
        <section className="u-stack u-gap-sm">
          ...
          <div className="admin-essay-editor__primary-fields">
            {primaryKeys.map((key) => {
              ...
              if (key === "tags") {
                return (
                  <label key={key} className="admin-essay-editor__field admin-essay-editor__field--summary" htmlFor="note-tags">
              ...
              if (key === "summary") {
                return (
                  <label key={key} className="admin-essay-editor__field admin-essay-editor__field--summary" htmlFor="note-summary">
              ...
              if (key === "title") {
                return (
                  <label key={key} className="admin-essay-editor__field admin-essay-editor__field--inline" htmlFor="note-title">
              ...
              if (key === "date") {
                return (
                  <label key={key} className="admin-essay-editor__field admin-essay-editor__field--inline" htmlFor="note-date">
      ...
      <section className="u-stack u-gap-sm">
        <h3 className="u-font-semibold u-text-sm u-letter-spaced">Note body</h3>
        <textarea
          name="body"
